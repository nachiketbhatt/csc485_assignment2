...
def lesk(sentence: Sequence[WSDToken], word_index: int) -> Synset:
    ...
    **IMPORTANT**: when looking up the word in WordNet, make sure you use the
    ...
    best_score = 0
    context = set([wsd.wordform.lower() for wsd in sentence])
    for synset in wn.synsets(sentence[word_index].lemma):
        signature = set()
        definition = synset.definition()
        examples = synset.examples()
        signature = signature.union(set(stop_tokenize(definition.lower())))
        for example in examples:
            signature = signature.union(set(stop_tokenize(example.lower())))
